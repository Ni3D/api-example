...
Выполните в командной строке: ```create-admin:dev``` для создания учётной записи администратора в базе для development режима, ```create-admin:prod``` - для создания учётной записи администратора в базе для production режима. (Логин и пароль задаются в файле ```.env```)
## Запуск приложения
